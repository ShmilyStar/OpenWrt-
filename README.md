...
DIY_P1_SH 为自定义设置1  
DIY_P2_SH 为自定义设置2  
###3.如需生成配置文件后再编译首先运行<生成配置文>Action，待运行成功后会在config目录下生成以设备名称命名的配置文件，然后运行<编译固件>Action，成功后会上传固件到release  
     如已有配置文件，将配置文件放在config目录下，运行<编译固件>Action，成功后会上传固件到release
